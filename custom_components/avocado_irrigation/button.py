...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    async_add_entities(
        [
            AvocadoButton(entry, controller, "run_deep_soak", "Run Deep Soak Now", controller.run_deep_soak),
            AvocadoButton(
                entry, controller, "run_routine", "Run Routine Irrigation Now", controller.run_routine_irrigation
            ),
            AvocadoButton(entry, controller, "reset_lock", "Reset Irrigation Lock", controller.reset_lock),
        ]
    )
