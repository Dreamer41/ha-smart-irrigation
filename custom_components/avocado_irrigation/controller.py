...
class AvocadoIrrigationController:
    ...
    async def async_setup(self) -> None:
        ...
        self._unsubs.append(
            async_track_time_change(
                self.hass,
                self._on_deep_soak_time,
                hour=self.deep_soak_time.hour,
                minute=self.deep_soak_time.minute,
                second=self.deep_soak_time.second,
            )
        )
        self._unsubs.append(
            async_track_time_change(
                self.hass,
                self._on_routine_time,
                hour=self.routine_time.hour,
                minute=self.routine_time.minute,
                second=self.routine_time.second,
            )
        )
    ...
    async def _on_power_restore(self, new_state: State) -> None:
        ...
        if new_state.state == "on":
            ...
            _LOGGER.info("Avocado irrigation: valve back online, confirmed %s (safe)", new_state.state)
    ...
    async def test_pulse(self, seconds: int) -> None:
        ...
        if self.store.state.lock_on:
            _LOGGER.warning("Avocado irrigation: test pulse skipped, lock already held")
            return
        await self._set_lock(True)
        await self._set_abort(False)
    ...
    async def _log_event(
        ...
        *,
        ...
        _LOGGER.info("Avocado irrigation event: %s (%s)", event_type, status)
        if extra_log:
    ...
    def _write_csv_row(self, row: list[str]) -> None:
        try:
            ...
            _LOGGER.exception("Could not write avocado irrigation CSV log to %s", self.csv_path)
